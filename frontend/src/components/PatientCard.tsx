...
export const PatientCard: React.FC<PatientCardProps> = ({ patient, onUpdate }) => {
  ...
  const getBmiBadge = (bmi?: number) => {
    ...
    if (bmi < 18.5) {
      label = "Дефицит веса";
      color = "bg-amber-950/30 text-amber-300 border-amber-900/40";
    } else if (bmi >= 25 && bmi < 30) {
      label = "Избыточный вес";
      color = "bg-zinc-800 text-zinc-200 border-zinc-700";
    } else if (bmi >= 30) {
      label = "Ожирение";
      color = "bg-rose-950/30 text-rose-300 border-rose-900/40";
    }
    return (
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl relative overflow-hidden">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center space-x-4">
            <div className="w-16 h-16 rounded-2xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-zinc-200 shadow-md">
          ...
          <button
            ...
            className="px-4 py-2 bg-zinc-800 hover:bg-zinc-750 text-zinc-200 rounded-xl text-sm font-medium flex items-center gap-2 border border-zinc-700 transition"
          >
      ...
      {/* Editing Form */}
      {isEditing && (
        <form onSubmit={handleSave} className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 shadow-xl space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="text-xs text-zinc-400 mb-1 block">ФИО</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs text-zinc-400 mb-1 block">Группа крови</label>
              <input
                type="text"
          ...
          <div className="space-y-3 pt-2">
            <div>
              ...
              <textarea
                ...
                className="w-full bg-zinc-950 border border-rose-900/50 rounded-xl px-3 py-2 text-sm text-zinc-100 focus:border-rose-700 outline-none"
              />
            </div>
          ...
          <div className="flex justify-end gap-3 pt-3">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-5 py-2 bg-zinc-100 hover:bg-white text-zinc-950 font-medium rounded-xl text-sm flex items-center gap-2 shadow-lg transition disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Chronic diseases */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
          <div className="flex items-center space-x-2 text-zinc-300 font-semibold text-sm mb-2">
