...
class LLMHub:
    ...
    def build_system_prompt(self, patient_profile: Dict[str, Any], context_sources: List[Dict[str, Any]]) -> str:
        ...
        prompt_parts = [
            ...
            "",
            "=== ПРАВИЛА АНАЛИЗА ===",
            "1. Все медицинские файлы, бланки анализов и сканы УЗИ пациента УЖЕ распознаны через OCR-систему и представлены ниже в блоке RAG в текстовом виде. Вы ОБЯЗАНЫ их детально изучить и разобрать!",
            "2. Никогда не заявляйте, что вы не можете просматривать изображения или файлы — все их содержимое уже извлечено для вас текстом.",
            "3. Назовите найденные биомаркеры (гемоглобин, ферритин, липопротеин(а), АСТ, эритроциты, данные УЗИ сосудов), их численные значения и референсы.",
            "4. Четко укажите, что в норме, а что повышено или понижено, и дайте клиническую оценку.",
            "5. Всегда учитывайте аллергии и текущую терапию пациента.",
        ]
